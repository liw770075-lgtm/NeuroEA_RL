"""Sequential static configuration with previous actions in the state."""
...
class ActionHistoryStaticConfigEnv(EnvBase):
    """Choose one parameter per step while exposing all previous choices."""
    ...
    def __init__(
        ...
        super().__init__()
        default_config = normalize_problem_config(
            problem_config,
        ...
        observation_dim = int(prototype_features.shape[0] + 2 * self.num_params)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.algorithm, self.problem, self._state = self._initialize_run(
            episode_seed, self.current_task
        )
        ...
        self._selected_mask = np.zeros(self.num_params, dtype=np.float32)
        self._done = False
        return self._build_observation(), self._build_info(
            self._state, reward=0.0, initial_update=True
        )
    ...
    def step(self, action):
        ...
        self._params[self._param_index] = action_value
        self._selected_mask[self._param_index] = 1.0
        self._param_index += 1
        if self._param_index < self.num_params:
            return self._build_observation(), 0.0, False, False, self._build_info(self._state)
        ...
        self._done = True
        return self._build_observation(), reward, True, False, self._build_info(
            final_state,
            reward=reward,
            update=real_params,
            normalized_update=self._params.copy(),
        )
    ...
    def _build_prototype(self):
        ...
        self.current_task, self.current_task_index = task, task_index
    ...
    def _initialize_run(self, seed, task):
        algorithm = build_stepneuroea_algorithm(
            task=task, device=self.device, dtype=self.dtype, seed=seed
        )
        problem = build_problem_instance(
            task=task, device=self.device, dtype=self.dtype, seed=seed
        )
        initial_update = resolve_stepneuroea_initial_update(
            self.initialization, algorithm, problem
        )
        return algorithm, problem, algorithm.reset(problem, update=initial_update)
    ...
    def _build_static_features(self, state):
        ...
        if self.include_task_context and state.get("task_context") is not None:
            parts.append(_to_numpy(state["task_context"], dtype=np.float32).reshape(-1))
        return np.concatenate(parts).astype(np.float32, copy=False)

    def _build_observation(self):
        return np.concatenate(
            [self._static_features, self._params, self._selected_mask]
    ...
    def _normalize_params(self, params):
        denominator = np.where(
            np.abs(self.upper_bounds - self.lower_bounds) < 1e-12,
            1.0,
            self.upper_bounds - self.lower_bounds,
        )
        return (2.0 * (params - self.lower_bounds) / denominator - 1.0).astype(np.float32)

    def _denormalize_params(self, params):
        params = np.clip(np.asarray(params, dtype=np.float32).reshape(-1), -1.0, 1.0)
        return self.lower_bounds + (params + 1.0) * 0.5 * (
            self.upper_bounds - self.lower_bounds
        )
